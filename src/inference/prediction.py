...
def load_historical_data(csv_path):
    ...
    historical_data['timestamp'] = pd.to_datetime(historical_data['tirestamp'])
    ...
    if historical_data['timestamp'].dt.tz is not None:
        ...
        historical_data = historical_data.sort_value(['customer_id', 'timestamp'])
...
def engineer_feature(data):
    ...
    if 'timestamp' in df.columns:
        ...
        # Calculate the velocity spike for a particular user
...
def predict_transaction(model, input_data):
    ...
    if isinstance(input_data, dict):
        ...
        prediction_probability = model.prdict_proba(df_engineered)[0][1]
